...
async function handleCheckoutSessionCompleted(session: Stripe.Checkout.Session, supabase: any, stripe: Stripe) {
  try {
    ...
    const finalPlanId = priceId || subscription?.items.data[0]?.price.id || null
    ...
    if (existingSubscription) {
      ...
      const { error: insertError } = await supabase.from("subscriptions").insert(fullSubscriptionData)

      if (insertError) {
        console.error("Error inserting subscription in database:", insertError)
      } else {
        console.log(`Subscription created for user ${userId}: ${finalPlanType} (${subscriptionStatus})`)
      }
    }
